...
export const PreviewPanel = ({
  ...
  const handleCreateFk = (toTableId: string, toColumnId: string) => {
    ...
    addForeignKey(selectedTableId, {
      name: `${selectedTableId}_fk_${toTableId}`,
      fromColumnId: selectedColumnId,
      toTableId,
      toColumnId,
    });
  };
      ...
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="flex gap-2">
            <button
              type="button"
              className={`${tabButton} ${
                activeTab === 'sql' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600'
              }`}
              ...
              {selected.fk ? (
                <div className="space-y-2">
                  ...
                  <div className="pt-2">
                    ...
                    <select
                      className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-sm"
                      value={selected.fk.startCardinality ?? ''}
                      onChange={(e) =>
                        updateForeignKey(selected.table.id, selected.fk!.id, {
                          startCardinality: (e.target.value as any) || undefined,
                        })
                      }
                    >
                      ...
                      <option value="one_and_only_one">Um (e apenas um)</option>
                      <option value="zero_or_one">Zero ou um</option>
                      <option value="one_or_many">Um ou muitos</option>
                      <option value="zero_or_many">Zero ou muitos</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-sm"
                      value={selected.fk.endCardinality ?? ''}
                      onChange={(e) =>
                        updateForeignKey(selected.table.id, selected.fk!.id, {
                          endCardinality: (e.target.value as any) || undefined,
                        })
                      }
                    >
                      <option value="">(auto)</option>
